...
class IndexAction extends Action {
     ...
     * 投票
      */
        ...
        public function mvote(){
            $count = 10;
            $page = 1;
            $map['eventId'] = 101085;
            $keyword = t($_REQUEST['searchKey']);
            if (strlen($keyword) > 0) {
                $map['realname'] = array('like', "%{$keyword}%");
                $_SESSION['vote_mvote'] = $keyword;
            }else{
                unset($_SESSION['vote_mvote']);
            }
            $list = D('EventPlayer', 'event')->getHandyList($this->mid, $map, $count, $page);
            $this->assign('list', $list);
            $this->display();
        }
        public function ajaxMvote(){
            $count = 10;
            $m=intval($_POST['n']);
            $page = ceil($m/$count)+1;
            $map['eventId'] = 101085;
            if (isset($_SESSION['vote_mvote']) && strlen($_SESSION['vote_mvote'])>0) {
                $map['realname'] = array('like', "%".$_SESSION['vote_mvote']."%");
            }
            $list = D('EventPlayer', 'event')->getHandyList($this->mid, $map, $count, $page);
            if(!$list){
                echo 0;die;
        ...
        public function doMvote(){
            $dao = D('EventPlayer', 'event');
            $pid = intval($_REQUEST['pid']);
            $result = $dao->vote($this->mid, $pid);
            if (!$result) {
